...
public sealed class UninstallRenoDXUseCase : IUninstallRenoDXUseCase
{
    ...
    public async Task<ModOperationResultDto> ExecuteAsync(Game game)
    {
        ...
        if (!result.IsSuccess)
        {
            ...
            {
                ErrorType.PermissionDenied => $"Permission denied uninstalling RenoDX from {game.Name}. Check you app permissions and try again.",
                ErrorType.FileSystemError => $"Failed to uninstall RenoDX from {game.Name}. The disk may be full or the file may be locked (game running?).",
